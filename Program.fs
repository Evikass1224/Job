...
let Errors (x:string) acc =
    let Rep_x = x.Replace(".", ",")
    match System.Double.TryParse(Rep_x) with
    | (true, value) -> value :: acc  //добавляем число в acc []
    | _ -> 
...
and AddElem listElem = 
    ...
    | (true, value) -> value :: listElem      //добавление элемента в начало списка
    | _ -> 
...
and DeleElem listElem = 
    ...
    | (true, value) -> 
        ...
        let newlistElem = Element value listElem
        newlistElem 
    | _ -> 
...
and SearchElem listElem = 
    ...
    | (true, value) -> 
        ...
        let rec Search elem list index =
            match list with
            | [] -> None 
            | h::t -> 
                if h = elem then 
                    Some index                //элемент найден
                else 
                    Search elem t (index + 1) //остальная часть списка

        match Search value listElem 1 with
        | Some index -> printfn "Элемент %f найден в списке на индексе %d." value index
        | None -> printfn "Элемент %f не найден в списке." value
...
and SecondLists listElem =
    ...
    let secondList = 
        ...
        |> Array.fold (fun acc x -> Errors x acc) []
        |> List.rev 
...
let rec input3 () =
    ...
    let listElem =
        ...
        |> Array.fold (fun acc x -> Errors x acc) []   //для каждого элемента вызываем функцию проверки на корректность acc = []
...
let rec input2 () =
    ...
    | (true, n) when n > 0 -> 
        ...
        if flag then
            ...
            printfn "В числе %d нет нечётных цифр." n
    | _ -> 
...
let input1 () =
    ...
    printfn "Список чисел, на 1 больших, чем вводимые значения: %A" result
    printfn ""
...
let rec Start () =
    ...
    | "2" -> input2()
    | "3" -> input3()
    | "q" -> printfn "Выход из программы." 
    | _ -> printfn "Ошибка! Входные данные не являются натуральным числом."  
    Start() 
